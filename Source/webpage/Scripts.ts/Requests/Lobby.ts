...
export function get_lobby_status(): object
{
	const lobby_uuid = window.location.pathname.split('/')[2];
	const url: string = `/api/lobby/${lobby_uuid}`;
...
export function update_player_color(color_id: number): object
{
	const lobby_uuid = window.location.pathname.split('/')[2];
	const url: string = `/api/lobby/${lobby_uuid}/player/color`;
	var xmlhttp = new XMLHttpRequest();
