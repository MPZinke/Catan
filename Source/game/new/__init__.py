...
def new_game(template_id: int) -> Game:
	...
	game_dict: dict = queries.games.new_game(template_id)  # pylint: disable=no-value-for-parameter
	...
	board_data: BoardData = new_board(game_dict["id"], template_data, random_resource_and_value_mapping)  # pylint: disable=no-value-for-parameter

	board: Board = construct_board(board_data)

	return Game(game_id, board, [])
